...
- (void)setupSubView {
    ...
    self.backButton.frame = CGRectMake(5, 40, 30,  30);
...
- (void)setupCamera {
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"提示",nil) message:NSLocalizedString(@"当前没有摄像头权限，请在设置中打开摄像头权限",nil) delegate:self cancelButtonTitle:NSLocalizedString(@"去设置",nil) otherButtonTitles:NSLocalizedString(@"取消",nil), nil];
        [alert show];
        return;
    }
